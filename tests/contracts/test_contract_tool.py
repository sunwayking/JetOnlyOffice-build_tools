...
class ContractToolTests(unittest.TestCase):
  ...
  def test_source_lock_accepts_reviewed_dictionary_license_expressions(self):
    for reviewed_expression in (
      ...
      "GPL-3.0-only AND MIT AND BSD-3-Clause AND BSD-2-Clause AND Zlib AND Libpng AND libpng-2.0 AND IJG AND libtiff AND CC0-1.0 AND LicenseRef-Public-Domain",
    ):
      value = source_lock()
